...
// what fish?
	...
	$('li.section-wrapper-block').has('a.column-close').css({
		'height' : '43px',
		'overflow' : 'hidden'
	});

	$('.section-wrapper-block .column-close').click(function(){
		if( $(this).parents('li').height() > 43 ){
			$(this).parents('li').css({
				'height' : '43px',
				'overflow' : 'hidden'
			});
	...
	var iconRender = $('.icon-selector-render');

	$('.icon-selector').each(function(e) {
		$(document).on('change', '.icon-selector', function(e) {
			$(this).parent().find('.current-icon').remove();
			var iconRender = $(this).closest('.icon-selector-render'); 
	 		var currentIcon = $(this).val();
			$(this).parent().find('.icon-selector-render').append( "<i class='current-icon fa fa-2x " + currentIcon + "'></i>" );	   
		});
	});
